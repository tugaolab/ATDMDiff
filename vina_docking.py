...
class QVinaDockingTask(BaseDockingTask):
    ...
    def __init__(self, pdb_block, ligand_rdmol, ligand_path, struct, conda_env='myDiffDec', tmp_dir='./tmp', use_uff=True, center=None):
...
/public/home/chensn/DL/DiffDec-master/autodocktools-prepare-py3k-master/prepare_receptor4.py -r {receptor_id}.pdb -o {receptor_id}.pdbqt
# Prepare ligand
...
def cal_vina_docking(
    ...
    for i in range(len(pred_list)):
        ...
        # Skip invalid SMILES strings or empty ones without returning
        if pred_mol is None or pred_smi == '':
            print(i, 'Invalid or empty SMILES, skipping.')
            result.append({
                'mol': pred_mol,
                'vina': None,
                'i': i
...
def cal_average_metric(
    ...
    with open(test_csv_path, 'r') as f:
        ...
        for row in csv_reader:
            if cnt % sample_num == 0:
                true_smi_list.append(row[1])  # SMILES string
                protein_filename_list.append(row[4])  # Protein filename
            cnt += 1

    # Step 2: Initialize vina metric dictionary
    vina_metric_dict = {true_smi_list[i] + protein_filename_list[i]: [] for i in range(len(true_smi_list))}
    ...
    for i in range(len(results)):
        ...
        if results[i]['vina'] and len(results[i]['vina']) != 0:
            key = true_smi_list[int(idx / sample_num)] + protein_filename_list[int(idx / sample_num)]
            vina_metric_dict[key].append(results[i]['vina'][0]['affinity'])

    # Step 4: Calculate metrics
    avg_tmp = []  # Store average scores for each group
    above_threshold_count = 0  # Count scores above the threshold
    total_scores = 0  # Total number of scores

    for v in vina_metric_dict.values():
        if len(v) == 0:
            continue
        avg_tmp.append(sum(v) / len(v))  # Average score for this group
        above_threshold_count += sum(1 for score in v if score > threshold)  # Count scores > threshold
    ...
    above_threshold_ratio = above_threshold_count / total_scores if total_scores > 0 else 0  # Proportion above threshold

    # Print results
    print('Overall vina score:', vina_score)
    print(f'Proportion of vina scores above {threshold}: {above_threshold_ratio * 100:.2f}%')
    print('=' * 30)

def cal_test_metric(
    test_csv_path = 'bingdingnet_test.csv',
...
def cal_high_aff(
    ...
    tot_cnt_dict = {}
    for i in range(len(true_smi_list)):
        key = true_smi_list[i] + protein_filename_list[i]
        high_aff_cnt_dict[key] = 0
        tot_cnt_dict[key] = 0
    ...
    for i in range(len(results_pred)):
        # 检查是否存在 'vina' 数据
        if 'vina' not in results_pred[i] or not results_pred[i]['vina']:
            print(f"Skipping index {i} in results_pred due to missing 'vina'")
            continue

        # 检查 results_testset[idx]['vina'] 是否存在
        idx = int(results_pred[i]['i'] / sample_num)
        if 'vina' not in results_testset[idx] or not results_testset[idx]['vina']:
            print(f"Skipping index {idx} in results_testset due to missing 'vina'")
            continue

        # 获取 vina_score 和 vina_ref
        try:
            vina_score = results_pred[i]['vina'][0]['affinity']
            vina_ref = results_testset[idx]['vina'][0]['affinity']
        except (IndexError, KeyError) as e:
            print(f"Error accessing 'vina' data at index {i}: {e}")
            continue

        # 更新高亲和力计数
        key = true_smi_list[idx] + protein_filename_list[idx]
        if vina_score <= vina_ref:
            high_aff_cnt_dict[key] += 1
        tot_cnt_dict[key] += 1
    ...
    if avg_tmp:
        ...
        print('No valid data for high affinity calculation.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_csv_path', action='store', type=str, required=False, default='formatted_fake_atom/bingdingnet_test.csv')
