...
def get_pocket(mol, pdb_path):
    ...
    contact_residues = np.unique(residue_ids[np.where(distances.min(1) <= 40)[0]])
...
def sample(checkpoint, samples_dir, data_dir, n_samples, task_name, device):
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        print(f'pocket mask shape:{pock_mask.shape}')
        #print(pock_mask)
        #exit()
        ...
        for i in tqdm(range(starting_point, n_samples), desc=str(batch_idx)):
            chain, node_mask, mean = model.sample_chain(data, sample_fn=sample_fn, keep_frames=1)#这里仔细看
